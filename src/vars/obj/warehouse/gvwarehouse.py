...
class WarehouseTag(BaseTag):
    ...
    def get_attributes_with_description(cls):
        return {
            ...
            "MAX_CLUSTER_COUNT": "Maximum number of clusters allowed when multi-cluster mode is enabled.",
            "MIN_CLUSTER_COUNT": "Minimum number of clusters allowed when multi-cluster mode is enabled.",
            "SCALING_POLICY": "Determines how additional clusters are added under load — 'STANDARD' (default) or 'ECONOMY'. (str)",
            "AUTO_SUSPEND": "Specifies the number of seconds of inactivity after which the warehouse is automatically suspended. (str)",
            "AUTO_RESUME": "If TRUE, resumes the warehouse automatically when a query is submitted. (str)",
            "INITIALLY_SUSPENDED": "If TRUE, creates the warehouse in a suspended state. (str)",
            ...
            "ENABLE_QUERY_ACCELERATION": "If TRUE, enables query acceleration for eligible queries. (str)",
            "QUERY_ACCELERATION_MAX_SCALE_FACTOR": "Maximum scale factor for compute resources used by query acceleration (1–8).",
            "MAX_CONCURRENCY_LEVEL": "Maximum number of concurrent SQL statements allowed on the warehouse.",
            "STATEMENT_QUEUED_TIMEOUT_IN_SECONDS": "Maximum time, in seconds, a SQL statement can be queued before timing out.",
            "STATEMENT_TIMEOUT_IN_SECONDS": "Maximum time, in seconds, before a running SQL statement times out.",
            ...
            "ENABLE_UNREDACTED_QUERY_SYNTAX_ERROR": "If TRUE, shows full unredacted query text in syntax error messages. (str)"
        }
