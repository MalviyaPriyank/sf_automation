...
class Databases:
    ...
    def get_owner_of_db(self,db_name):
        df = self.session.table(self.view).filter(col("database_name") == '{db_name}').select(col("database_owner"))
        res = df.collect()
        return res[0][0]
    
    def get_type_of_db(self,db_name):
        df = self.session.table(self.view).filter(col("database_name") == '{db_name}').select(col("type"))
        res = df.collect()
        return res[0][0]
